...
def _driver_lines(prefix, configure_flags, make_flags, build_targets, install_target, install_commands, cc_flags, exports, extra_setup, libc_headers):
    ...
    lines += [
        ...
        "# The wait has to be in whole seconds, because that is the",
        "# resolution the check compares at: it asks `ls -t`, and this",
        "# coreutils records only seconds. bash's own -nt would not do -- it",
        "# compares nanoseconds, so a loop waiting on it finishes immediately",
        "# and leaves the two files inside the same second after all. SECONDS",
        "# is bash's counter and moves with the clock, and it needs no",
        "# external program: coreutils' sleep is built against mes-libc here",
        "# and cannot read the realtime clock.",
